...
📊 **训练与实验数据汇总见 [`RESULTS.md`](RESULTS.md)** —— PKU-Market-PCB 6 类缺陷检测。
> 最终结果（test 集，母板 9-10，完全未见过）：**mAP50 = 0.8191**，mAP50-95 = 0.3554，P = 0.8291，R = 0.7772。

> ⚠️ **重要方法学修正**：早期 mAP50 = 0.9266 **因数据泄漏而虚高**。
> 实测发现该数据集只有 10 块 PCB 母板，同母板的图在非缺陷区域**逐像素完全相同**。
> 已改为按母板 block 划分（整块板只进一个 split）并加了自动泄漏自检。
> 详见 `RESULTS.md` 顶部勘误。
